        ...
        <div class="flex items-center justify-center">
            <div class="nav-container flex items-center gap-2 rounded-xl p-1.5 shadow-xl">
                ...
                <a href="{{ route('home') }}"
                    ...
                    aria-label="Go to home" aria-current="{{ $currentRoute === 'home' ? 'page' : 'false' }}">
                ...
                <a href="{{ route('info') }}"
                    ...
                    aria-label="View information" aria-current="{{ $currentRoute === 'info' ? 'page' : 'false' }}">
                    <svg class="transition-transform hover:scale-110" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const navHistory = {
                ...
                init() {
                    ...
                    const lastUrl = this.stack[this.stack.length - 1];
                ...
                push(url) {
                    ...
                    if (clean !== last) {
                        this.stack.push(clean);
                        if (this.stack.length > 20) this.stack.shift(); // limit size
                        this.save();
                    }
                },

                getPrevious() {
                    const current = this.cleanUrl(window.location.href);
                    ...
                    while (this.stack.length > 0) {
                        ...
                        if (prev !== current) {
                            ...
                            this.stack.pop(); // remove duplicates
                        }
                    }
                ...
                load() {
                    try {
                        const data = localStorage.getItem('navHistory');
            ...
            if (backBtn) {
                backBtn.addEventListener('click', function (e) {
                    ...
                    if (prev && prev !== window.location.href) {
                        ...
                        window.location.href = "{{ route('homepage') }}"; // fallback
                    }
                });
            }
            ...
            document.querySelectorAll('.nav-item').forEach(item => {
                item.addEventListener('click', function (e) {
                    if (
                        ...
                        // Push current before navigating
            ...
            // Optional: Save again when user comes back from another tab
